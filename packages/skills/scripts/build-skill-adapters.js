...
const SKILL_NAMES = ['n8n-architect'];
const LEGACY_SKILL_NAMES = ['n8n-manager'];
...
function removeOrCheckLegacySkills() {
  const roots = [DIST_DIR, ...MIRROR_SKILL_ROOTS, ...PLUGIN_SKILL_ROOTS];
  for (const root of roots) {
    for (const skillName of LEGACY_SKILL_NAMES) {
      const skillDir = path.join(root, skillName);
      if (checkMode) {
        if (fs.existsSync(skillDir)) {
          throw new Error(`Legacy generated skill should not exist: ${path.relative(WORKSPACE_ROOT, skillDir)}`);
        }
      } else {
